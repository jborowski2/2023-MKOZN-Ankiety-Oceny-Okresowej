                ...
                try
                {
                    ...
                    foreach (var strona in ankieta.StronyAnkiet)
                    {
                        if (strona.PolaAnkiety != null)
                        {
                            ...
                            {
                                if (pole.Attachment != null && pole.Attachment.File != null && pole.Attachment.File.ContentLength > 0 && pole.Attachment.File.ContentType == "application/pdf")
                                {
                                    Attachment attachment = new Attachment();
                                    string originalFileName = Path.GetFileNameWithoutExtension(pole.Attachment.File.FileName);
                                    string fileExtension = Path.GetExtension(pole.Attachment.File.FileName);
                                    string uniqueFileName = originalFileName + "_" + DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss") + fileExtension;

                                    string filePath = Path.Combine(serverFolderPath, uniqueFileName);
